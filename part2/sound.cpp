...
Sound::Sound(DATAFILE *data) {
	...
	_bgm = (SAMPLE *) _data[HARDLINE_BGM_WAV].dat;
	if (!_bgm) {
	...
	_soundEffectTitle = HIT_SFX_WAV;
	_soundEffect = (SAMPLE *) _data[HIT_SFX_WAV].dat;
	if (!_soundEffect) {
...
void Sound::setBGM(SAMPLE *sound) {
	_bgm = sound;
	if (!_bgm) {
		allegro_message("Error Loading Music");
	}
